...
#import "IFLYRouterDefine.h"
@interface IFLYRouterManagement()
/**路由信息数据字典*/
@property(nonatomic,strong) NSMutableDictionary *plistdata;
@end
@implementation IFLYRouterManagement
+(id)sharedInstance {
    static dispatch_once_t onceToken;
    static IFLYRouterManagement * router;
    dispatch_once(&onceToken,^{
        router = [[IFLYRouterManagement alloc] init];
        //解析路由配置文件，生成路由配置字典。plist文件中的 key 就是对应控制器的简称，这个可以自由定义；而value则是对应控制器的类名，这个必须和你创建的controller一致，否则会报错
        NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"IFLYRouterData" ofType:@"plist"]; //用来存放路由跳转的一些控制器类名
